...
| Vercel | Hosting | ✅ **Production is `carbonify-gilt.vercel.app`** (verified 2026-08-05 across all 106 deployed chunks), deployed by the Vercel **Git integration** on push to `main`. ⚠️ **Not `carbonify13.vercel.app`** — that 404s since the GitHub repo was renamed. ⚠️ **At least two other projects exist**: `carbonify.vercel.app` (an unrelated React app, confusingly titled "Carbonify") and `ecolink.vercel.app` (a bare Vite starter) — see [VERCEL_DOMAIN_AND_REDEPLOY.md](VERCEL_DOMAIN_AND_REDEPLOY.md) |
