...
| ☐ OWN-04 | The **7 required** edge functions are deployed | Dashboard → Edge Functions | `account-deletion` · `paymongo-checkout` · `paymongo-reconcile` · `paymongo-resettle` · `paymongo-webhook` · `process-payouts` · `send-approval-email`. ⚠️ **This row said 8 until 2026-08-05.** The eighth, `public-registry`, is **deliberately not deployed** — an ungated white-label scaffold nothing in `src/` calls. **Do not deploy it to tick this box**; see [SOFT_LAUNCH_RUNBOOK](SOFT_LAUNCH_RUNBOOK.md) §1c |
