...
| Layer | State |
|---|---|
| Unit tests | ✅ **1303 passing across 115 files** (Vitest) — re-measured 2026-08-05 (evening), not carried forward. *(This row read "920 across 79 files, re-run 2026-08-01" until then — 383 tests and five days of drift, on the page that owns what is tested. A count is a claim like any other.)* Pure math: fees, VAT, reconciliation logic, farmer/investor/MRV aggregation. The 2026-07-22 role audit added ~200, covering the VER calculation breakdown, EXIF/evidence integrity, LGU jurisdiction matching, AML name screening, admin segregation of duties and the verification timeline. |
