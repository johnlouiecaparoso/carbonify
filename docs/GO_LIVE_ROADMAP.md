...
| ~~**Email confirmation is OFF by choice**~~ — **corrected 2026-07-30: it is ON.** `pilot-readiness.spec.js` read GoTrue `/auth/v1/settings` and found `mailer_autoconfirm` = `false` (confirmation required) **and** `disable_signup` = `true` — both the opposite of what this row claimed, and the second one blocks the pilot outright. What remains is the *domain*: until a sender domain is verified in Resend, confirmation mail goes via Supabase's shared default SMTP, so it lands in spam. Needs a domain (~₱600–900/yr), not a subscription. | Dashboard + domain | 🟡 signups must be re-enabled before the beta — [YOUR_ACTION_ITEMS](YOUR_ACTION_ITEMS.md) Step 2 |
