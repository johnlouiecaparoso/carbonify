...
> > | `public-registry` | 🔴 **not redeployed** | **Re-confirmed 2026-08-08:** still `404 NOT_FOUND` at the gateway, identical to a made-up function name run as a control. Now needs `SUPABASE_SERVICE_ROLE_KEY`; without it the anonymous tier still serves and keyed calls 401. 🆕 **The first deploy attempt FAILED 2026-08-08** — `Module not found ".../source/routing.ts"`. The version routing had been split into a second file; the bundler had only `index.ts`. Now **inlined, one file**, with a test that fails if a relative import returns (mutation-checked). *The flagged risk — "the bundle is unverified until the first deploy" — was the correct call and the deploy is what measured it* |
