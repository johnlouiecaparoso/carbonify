...
| 15 | **Error handling: the remaining half** — services still swallow vs throw inconsistently. Everything else this row used to claim is closed: `ErrorBoundary` is mounted and uses `errorStore` in full, the `main.js` monkeypatches went 2026-07-29, and the nullable-client race behind the split was fixed at the root 2026-08-01. The 07-30 → 08-02 passes closed most of the remaining surface read by read | [#15](DEFERRED_BACKLOG.md) |
