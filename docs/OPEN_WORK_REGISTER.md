...
| **Staff read posture** | 🟡 🆕 **written 2026-08-05, owner-run**: [`staff_profile_reads.sql`](../supabase/diagnostics/staff_profile_reads.sql). Read-only, finds an admin and a verifier itself, **nothing to edit**. Answers the half the negative suite structurally cannot: that file impersonates a NON-admin on purpose, so all three of its runs said nothing about whether `kycService` / `amlService` / `projectApprovalService` can see the people they review. ⚠️ **It reads the opposite way** — rows visible means the queues work, where in the negative suite rows visible is a breach | [#39](DEFERRED_BACKLOG.md) |
