...
| [ESCROW_TEST_RUNSHEET.md](ESCROW_TEST_RUNSHEET.md) | 🆕 **The team run sheet for `ESC-01…06`, the last gate before the beta.** UAT says *what* to test; this says *who does what, in what order, and what to hand to the next person* — because each claim is visible on a screen only the buyer, the seller or the owner can see. Owner set-up, 16 steps with handoffs, a one-pager per helper, a feedback sheet. Also corrects `ESC-03`, which could not have passed as written |
