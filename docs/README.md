...
| [OWNER_TEST_GUIDE.md](OWNER_TEST_GUIDE.md) | 🆕 **Yours, for the test session.** Set-up before anyone arrives, the 16-step running order with handoff points, every SQL statement, the admin actions, and the close-out list. Do **not** hand this to a tester — it contains the expected answers, and knowing them in advance changes what people report |
| [TESTER_GUIDE.md](TESTER_GUIDE.md) | 🆕 **Hand this to each helper.** Plain language, no technical knowledge assumed, no jargon or symbols. Covers the group money test by role, the per-role tests, and the keyboard / phone / logged-out checks. Leads with the ask that matters most: *if a screen says you have nothing and you know that is wrong, write it down* |
| [TESTER_FEEDBACK.md](TESTER_FEEDBACK.md) | 🆕 **What each helper fills in and sends back.** Fill-in blanks rather than prose, so the answers come back in a shape that can actually be read. Six questions that each target a bug class the automated suite cannot see |
| [ESCROW_TEST_RUNSHEET.md](ESCROW_TEST_RUNSHEET.md) | ↳ *Replaced 2026-08-05 by the three above — kept as a pointer.* It mixed owner SQL with tester instructions, so neither audience could be handed it |
