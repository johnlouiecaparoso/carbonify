...
--   public.mark_payout_processing()  ->  20260718000300_payout_processing_returns_claim.sql
--
...
begin
  select string_agg(msg, chr(10)) into v_blocked from (
      ...
      union all
      select 'mark_payout_processing — recover by re-applying 20260718000300_payout_processing_returns_claim.sql' as msg
       where exists (
         select 1 from pg_proc p
           join pg_namespace n on n.oid = p.pronamespace
          where n.nspname = 'public'
            and p.proname = 'mark_payout_processing'
            and pg_get_functiondef(p.oid) like '%v_updated = 1%')
  ) t;
  ...
  then
    raise exception using
      ...
      message = 'REFUSING TO RUN 20260606000700_payout_requests.sql — a NEWER definition is already live and this file would silently revert it',
      detail  = v_blocked,
