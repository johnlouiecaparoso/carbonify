...
begin
  select string_agg(msg, chr(10)) into v_blocked from (
      select 'assert_can_trade — recover by re-applying 20260722000800_account_suspension.sql' as msg
       where exists (
         select 1 from pg_proc p
           join pg_namespace n on n.oid = p.pronamespace
          where n.nspname = 'public'
            and p.proname = 'assert_can_trade'
  ...
  then
    raise exception using
      ...
      message = 'REFUSING TO RUN 20260721000100_assert_can_trade.sql — a NEWER definition is already live and this file would silently revert it',
      detail  = v_blocked,
